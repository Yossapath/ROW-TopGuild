...
export default function TeamsPage() {
        ...
        <div className="bg-theme-panel rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col border border-theme-border animate-in zoom-in-95 duration-200">
          ...
          <div className="p-4 flex-1 flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={handlePullTop60} className="text-[#0b3d63] dark:text-white font-bold text-sm bg-[#0b3d63]/10 dark:bg-[#0b3d63]/20 px-4 py-1.5 rounded-lg hover:bg-[#0b3d63]/20 dark:hover:bg-[#0b3d63]/30 transition-colors border border-[#0b3d63]/20 dark:border-[#0b3d63]/40 flex items-center gap-1">
                ✨ ดึง 60 พลังสูงสุดมาวางให้ก่อน
              </button>
            </div>
            <textarea 
              className="w-full h-[250px] bg-theme-bg border border-theme-border rounded-lg p-3 text-sm text-theme-text font-mono resize-none focus:ring-2 focus:ring-[#0b3d63] dark:focus:ring-[#0b3d63] outline-none"
              value={autoModalText}
  ...
  return (
    <div className="space-y-6 bg-[#f0f6fc] dark:bg-[#1c1c1c] min-h-screen p-4 lg:py-8 lg:px-12 xl:px-24 2xl:px-32 relative" style={{ zoom: 0.85 }}>
      ...
      {/* Header Card */}
      <div className="bg-white dark:bg-[#242424] rounded-2xl shadow-sm border border-slate-200 dark:border-[#333333] p-5 mb-5 flex flex-col lg:flex-row items-center justify-between gap-4">
        ...
        {isAdmin && (
          <div className="flex items-center gap-3 w-full lg:w-auto justify-end flex-wrap">
            <button onClick={handleClearAll} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#2a2a2a] text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900/50 rounded-xl font-bold hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors text-sm shadow-sm">
              <span>ล้างทั้งหมด</span>
            </button>
            <button onClick={() => setIsAutoModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#2a2a2a] text-[#0b3d63] dark:text-white border border-[#0b3d63] dark:border-[#0b3d63] rounded-xl font-bold hover:bg-blue-50 dark:hover:bg-sky-950/30 transition-colors text-sm shadow-sm">
              <Wand2 size={16} /> <span>ออโต้จัดทีม (Auto)</span>
            </button>
            <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-2 px-5 py-2 bg-[#0b3d63] hover:bg-[#0f4b7a] text-white rounded-xl font-bold transition-colors shadow-sm disabled:opacity-50 text-sm">
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="flex gap-4 items-start">
            {isAdmin && !isUnassignedCollapsed && (
              <div className="w-[300px] flex-shrink-0 bg-white dark:bg-[#242424] rounded-2xl shadow-sm border border-slate-200 dark:border-[#333333] h-[calc(100vh-2rem)] flex flex-col sticky top-4 z-10 transition-all">
                <div className="p-3 border-b border-slate-100 dark:border-[#333333] bg-slate-50/70 dark:bg-[#2a2a2a]/50 rounded-t-2xl">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={() => setIsUnassignedCollapsed(true)} className="text-slate-400 dark:text-gray-400 hover:text-slate-700 dark:hover:text-slate-200 bg-white dark:bg-[#2a2a2a] border border-slate-200 dark:border-[#333333] rounded-lg p-1"><ChevronLeft size={14}/></button>
                  </div>
                  <div className="space-y-2">
                    <input
                      ...
                      className="w-full bg-white dark:bg-[#2a2a2a] border border-slate-200 dark:border-[#333333] rounded-lg px-2.5 py-1.5 text-xs font-medium text-slate-800 dark:text-white placeholder:text-slate-400 dark:placeholder:text-gray-500 outline-none focus:ring-2 focus:ring-[#0b3d63] dark:focus:ring-[#0b3d63]"
                    />
                    <select 
                      value={unassignedFilterJob} 
                      onChange={e => setUnassignedFilterJob(e.target.value)}
                      className="w-full bg-white dark:bg-[#2a2a2a] border border-slate-200 dark:border-[#333333] rounded-lg px-2.5 py-1.5 text-xs font-bold text-slate-800 dark:text-white outline-none"
                    >
                ...
                <Droppable droppableId="unassigned" type="MEMBER">
                  {(provided, snapshot) => (
                    <div ref={provided.innerRef} {...provided.droppableProps} className={`flex-1 overflow-y-auto p-2 space-y-1.5 transition-colors ${snapshot.isDraggingOver ? 'bg-blue-50/50 dark:bg-[#0b3d63]/25' : ''}`}>
            ...
            {isAdmin && isUnassignedCollapsed && (
              <div className="flex-shrink-0 bg-white dark:bg-[#242424] rounded-2xl shadow-sm border border-slate-200 dark:border-[#333333] h-[calc(100vh-2rem)] flex flex-col items-center py-4 sticky top-4 z-10 w-12 cursor-pointer hover:bg-slate-50 dark:hover:bg-[#2e2e2e] transition-colors" onClick={() => setIsUnassignedCollapsed(false)}>
                <ChevronRight size={20} className="text-slate-400 mb-4"/>
                <Users size={18} className="text-slate-400 mb-2"/>
                <span className="bg-[#0b3d63] dark:bg-[#0b3d63] text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{data.columns["unassigned"].memberIds.length}</span>
              </div>
            )}

          <div className="flex-1 min-w-0 flex flex-col">
            <div className="flex gap-2 mb-4 bg-white dark:bg-[#242424] p-1.5 rounded-xl border border-slate-200 dark:border-[#333333] shadow-sm self-start">
              <button 
                onClick={() => setActiveTab("main")}
                className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'main' ? 'bg-[#0b3d63] dark:bg-[#0b3d63] text-white shadow-sm' : 'text-slate-600 dark:text-white hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-[#2e2e2e]'}`}
              >
                สนามหลัก (60 คน)
              </button>
              <button 
                onClick={() => setActiveTab("sub")}
                className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'sub' ? 'bg-[#0b3d63] dark:bg-[#0b3d63] text-white shadow-sm' : 'text-slate-600 dark:text-white hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-[#2e2e2e]'}`}
              >
              ...
              {isAdmin && (
                <button 
                  ...
                  className={`px-6 py-2 rounded-lg font-bold text-sm transition-all ${activeTab === 'leave' ? 'bg-red-600 text-white shadow-sm' : 'text-slate-600 dark:text-white hover:text-slate-900 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-[#2e2e2e]'}`}
                >
                  ลา/ออฟไลน์
                </button>
              )}
  ...
  return (
    <Draggable draggableId={column.id} index={index} isDragDisabled={!isAdmin}>
      {(providedTeam, snapshotTeam) => (
        <div ref={providedTeam.innerRef} {...providedTeam.draggableProps} className={`bg-white dark:bg-[#242424] rounded-2xl shadow-sm border overflow-hidden ${snapshotTeam.isDragging ? 'shadow-xl ring-2 ring-[#0b3d63] dark:ring-[#0b3d63] border-[#0b3d63] dark:border-[#0b3d63] z-50' : 'border-slate-200 dark:border-[#333333]'} ${column.locked ? 'opacity-95 border-amber-400 dark:border-amber-500' : ''}`}>
          {/* Header */}
          ...
          <div className="grid grid-cols-[30px_1fr_120px_70px_30px] gap-2 px-3 py-2 bg-slate-50 dark:bg-[#2a2a2a]/60 border-b border-slate-100 dark:border-[#333333] text-[11px] font-bold text-slate-500 dark:text-gray-400">
            <div></div>
            <div>ชื่อ</div>
          ...
          <div className="p-2 min-h-[220px] flex flex-col gap-1.5 relative z-10 bg-white dark:bg-[#242424]">
                  ...
                  <Droppable key={droppableId} droppableId={droppableId} type="MEMBER" isDropDisabled={!isAdmin || column.locked}>
                    {(provided, snapshot) => (
                      <div 
                        ...
                        className={`h-[34px] rounded-lg border ${snapshot.isDraggingOver ? 'bg-blue-50 dark:bg-[#0b3d63]/25 border-[#0b3d63] dark:border-[#0b3d63]' : 'border-transparent bg-slate-50/70 dark:bg-[#2a2a2a]/40'} flex items-center relative transition-colors`}
                      >
                         {!memberId && !snapshot.isDraggingOver && (
                           <div className="absolute inset-0 border border-dashed border-slate-200 dark:border-[#333333] rounded-lg flex items-center justify-center bg-transparent pointer-events-none">
                             <span className="text-[10px] text-slate-400 dark:text-gray-500 font-bold tracking-widest">ว่าง {slotIdx + 1}</span>
                           </div>
                         )}
                         ...
                         {memberId && (
                           <Draggable draggableId={memberId} index={0} isDragDisabled={!isAdmin || column.locked}>
                             {(prov, snap) => {
                                ...
                                return (
                                  <div ref={prov.innerRef} {...prov.draggableProps} className={`absolute inset-0 w-full h-full grid grid-cols-[30px_1fr_120px_70px_30px] gap-2 items-center px-1 py-1 rounded-lg bg-white dark:bg-[#2a2a2a] hover:bg-slate-50 dark:hover:bg-[#2e2e2e] group border border-slate-200 dark:border-[#333333] ${snap.isDragging ? 'shadow-lg border-blue-400 dark:border-[#0b3d63] ring-2 ring-[#0b3d63]/20 dark:ring-[#0b3d63]/20 z-50' : 'shadow-sm'}`} style={prov.draggableProps.style}>
                                    <div className="flex items-center justify-center text-slate-400 dark:text-gray-500 cursor-grab" {...(isAdmin ? prov.dragHandleProps : {})}>
                                      {isAdmin ? <GripVertical size={14}/> : <span className="text-[10px] text-slate-400 dark:text-gray-500 font-mono">#{slotIdx+1}</span>}
                                    </div>
                                    ...
                                    {isAdmin && (
                                      <button onClick={() => removeMember(column.id, memberId)} disabled={column.locked} className="text-slate-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity flex justify-center disabled:hidden"><X size={14}/></button>
                                    )}
                                  </div>
                                )
                             }}
      ...
      {(provided, snapshot) => (
        <div
          ...
          className={`flex items-center justify-between p-2 rounded-xl border shadow-sm select-none transition-all ${
            snapshot.isDragging ? 'shadow-lg border-[#0b3d63] dark:border-[#0b3d63] z-50 ring-2 ring-[#0b3d63]/20 dark:ring-[#0b3d63]/20 bg-white dark:bg-[#2a2a2a]' : 'border-slate-200 dark:border-[#333333] hover:border-slate-300 dark:hover:border-slate-600'
          }`}
          style={{
