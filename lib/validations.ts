...
// Denormalized display slot used in the "main"/"sub" preview arrays sent by
// the client (one entry per seat in a column, null when the seat is empty).
const teamDisplaySlotSchema = z
  .object({
...
export const teamDataSchema = z.object({
  // Optional: the Teams page's handleSave() does not currently send `members`
  // (it's derived/looked up server-side from columns instead), so this can't
  // be a required field or every save gets rejected with a 400.
  members: z
    .record(
      z.string(),
      z.object({
  ...
  main: z.array(z.array(teamDisplaySlotSchema)).optional(),
  sub: z.array(z.array(teamDisplaySlotSchema)).optional(),
});
