...
export function isBookingOpen(schedule?: {
  ...
  // No schedule object at all means no restrictions have been configured —
  // treat it the same as an "unlimited" schedule (matches the caller in
  // /api/dungeon/queues, which only runs this check when a schedule doc
  // actually exists; a missing schedule is otherwise never restrictive).
  if (!schedule) {
    return { open: true };
  }

  // openDate is the master switch: if it isn't meaningfully set, the
  // schedule is unrestricted (open), regardless of whether openTime/
  // closeTime happen to be filled in.
