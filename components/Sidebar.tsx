...
export default function Sidebar({ isExpanded }: { isExpanded: boolean }) {
  ...
  return (
    <aside 
      className={`flex-shrink-0 bg-[#0b3d63] h-screen flex flex-col transition-all duration-300 shadow-xl border-r border-[#082e4b] z-30 ${
    ...
    >
      <div className={`p-5 flex flex-col justify-center min-h-[64px] border-b border-white/10 ${isExpanded ? "items-start" : "items-center"}`}>
        <div className="flex items-center space-x-2">
          <Settings size={22} className="text-blue-200 flex-shrink-0" />
          {isExpanded && <h2 className="font-extrabold text-xl tracking-tight text-white uppercase whitespace-nowrap">TOPGUILD OS</h2>}
        </div>
        {isExpanded && <p className="text-blue-200 opacity-80 text-xs mt-1 ml-8 whitespace-nowrap font-medium">Guild Management</p>}
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center transition-all ${
                ...
                isActive
                  ? "bg-white text-[#0b3d63] font-bold shadow-md"
